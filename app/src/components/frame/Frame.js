...
export default function Frame(){
    ...
    const saveIdToFirebase = (cvalue) => {
        ...
        const collectionRef = projectFirestore.collection('frame'+cvalue).add({
            name: "",
            userId: "",
            createdAt: createdAt,
        });
    ...
    return (
        <div>
            <h1>foto's</h1>
            { docs && docs.map( doc => (
                <p key={doc.id}>Dit toestel is nog niet toegevoegd aan een account. Ga naar jouw account, vervolgens naar devices en voeg volgende code in om het device te linken: <span><br></br><br></br>{frameID}</span></p>
            ))}
        </div>
    )
}
